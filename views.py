			...
			for goal in goals:
				if goal.frequency == "Daily":
					...
					if goal.goal_type == "Sleep":
						progresses.append([pytz.utc.localize(progress.date_recorded).astimezone(pytz.timezone(user.timezone)).strftime("%I:%M%p %B %d, %Y"), round(mean_value/(60.0*60),2), min(100.0*mean_value/goal.value,100), goal,pytz.utc.localize(goal.valid_to).astimezone(pytz.timezone(user.timezone))])
					else:
						progresses.append([pytz.utc.localize(progress.date_recorded).astimezone(pytz.timezone(user.timezone)).strftime("%I:%M%p %B %d, %Y"), round(mean_value,2), min(100.0*mean_value/goal.value,100), goal,pytz.utc.localize(goal.valid_to).astimezone(pytz.timezone(user.timezone))])
				else:
					progress = goal.get_current_status()
					if goal.goal_type == "Sleep":
						progresses.append([pytz.utc.localize(progress.date_recorded).astimezone(pytz.timezone(user.timezone)).strftime("%I:%M%p %B %d, %Y"), progress.value/(60.0*60), min(100.0*progress.value/goal.value,100), goal ,pytz.utc.localize(goal.valid_to).astimezone(pytz.timezone(user.timezone))])
					else:
						progresses.append([pytz.utc.localize(progress.date_recorded).astimezone(pytz.timezone(user.timezone)).strftime("%I:%M%p %B %d, %Y"), progress.value, min(100.0*progress.value/goal.value,100), goal,pytz.utc.localize(goal.valid_to).astimezone(pytz.timezone(user.timezone))])
	...
	if session.get("user_id"):
		...
		for goal in goals:
			if goal.frequency == "Daily":
				...
				print series
			goal_dict[goal.goal_id] = {"series": series, "valid_from": pytz.utc.localize(goal.valid_from).astimezone(pytz.timezone(user.timezone)).strftime("%Y-%m-%d %H:%M:%S"),
										"valid_to": pytz.utc.localize(goal.valid_to).astimezone(pytz.timezone(user.timezone)).strftime("%Y-%m-%d %H:%M:%S"),"value":goal.value,
